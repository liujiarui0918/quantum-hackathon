...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    summary = {
        ...
        "label_key": args.label_key,
        "model_scope": "non_neural_linear_ranker",
    }
    args.summary.parent.mkdir(parents=True, exist_ok=True)
    args.summary.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"wrote block model: {args.output}")
    print(f"training rows: {len(y)}")
    print("model_scope: non_neural_linear_ranker")
    return 0


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a non-neural route7++ linear block scorer from trace JSONL.")
