...
\operatorname*{subject\ to} x_{a} + x_{b} + x_{c} = 1
\end{aligned}
\]
...
\item 罚函数 penalty
\item 显式编码 encoding
\item 不等式 slack / unbalanced penalty
\item 求解后 feasibility check + repair
\item 约束保持 mixer
\end{enumerate}

前四个属于这条路线。第五个会在路线 5 里单独讲。

\subsection*{等式约束：平方罚项}
...
\[
A \cdot  (\sum_{i} x_{i} - k)^2
\]

这种方法简单、通用、容易解释。缺点是：展开后会产生很多二次耦合，而且 penalty 权重不好选。

\subsection*{At-most-one：互斥罚项}
...
\[
A \cdot  (x_{a} x_{b} + x_{a} x_{c} + x_{b} x_{c})
\]
...
\subsection*{不等式约束：slack 的直觉}

预算约束：

\[
5 x_{a} + 3 x_{b} + 4 x_{c} + 2 x_{x} + x_{y} \le 6
\]

可以加入 slack 变量 \texttt{s >= 0}，改成等式：

\[
5 x_{a} + 3 x_{b} + 4 x_{c} + 2 x_{x} + x_{y} + s = 6
\]

然后罚：

\[
A \cdot  (5 x_{a} + 3 x_{b} + 4 x_{c} + 2 x_{x} + x_{y} + s - 6)^2
\]

slack 的问题是变量会增加。预算上限越大，slack 编码越重。很多实际问题卡住，不是因为目标函数复杂，而是因为 slack 变量把 QUBO 撑爆了。

\subsection*{Unbalanced penalty 的直觉}
...
如果 \texttt{lhs} 小于 \texttt{rhs}，其实不用罚；只有超过才罚。
...
\subsection*{Encoding：把合法性写进变量结构}
...
\subsection*{Feasibility checker}
...
\begin{itemize}
\item feasible ratio
\item best feasible objective
\item best raw energy 是否可行
\item 求解时间
\item 系数范围
\end{itemize}
...
拿 \texttt{sample\_problem.json} 做三件事：

\begin{enumerate}
\item 写出 \texttt{choose\_one\_model} 的 penalty。
\item 想一想 \texttt{resource\_budget} 用 slack 会增加多少变量。
\item 设计一个预算超限 repair：如果超预算，你删哪个变量，为什么？
\end{enumerate}
...
\begin{itemize}
\item \texttt{literature/requirements/02\_constraint\_handling\_requirements.md}
\item Hen \& Spedalieri, constrained optimization with quantum annealing
\item Montanez-Barrera et al., unbalanced penalization for inequalities
\item Chancellor, domain-wall encoding
\end{itemize}
