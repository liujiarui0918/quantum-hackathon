...
\section*{路线 3：Quantum Annealing / Simulated Annealing 求解}
...
如果变量有 \texttt{n} 个，可能的 bitstring 有 \texttt{2\textasciicircum{}n} 个。小问题可以 brute force，全枚举。大一点就不行。
...
\[
H(t) = A(t) \cdot  H_{initial} + B(t) \cdot  H_{problem}
\]

开始时 \texttt{H\_initial} 强，系统容易准备。结束时 \texttt{H\_problem} 强，系统希望落在问题的低能态。
...
\begin{itemize}
\item best raw energy sample
\item best raw energy sample 是否可行
\item best feasible sample
\item feasible ratio
\item repair 后目标
\item 和 exact / random / greedy 的对比
\end{itemize}
...
想象我们对 \texttt{sample\_problem.json} 跑 simulated annealing。

你需要设计结果表格的字段：

\begin{itemize}
\item raw bitstring
\item raw QUBO energy
\item decoded solution
\item original objective
\item feasible
\item violation detail
\item repaired solution
\end{itemize}
...
\begin{itemize}
\item \texttt{literature/requirements/03\_annealing\_solver\_requirements.md}
\item Phillipson \& Bausch, quantum annealing industry review
\item D-Wave Ocean 文档中的 BQM / sampler 概念
\end{itemize}
